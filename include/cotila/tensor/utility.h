 ...
 */
  ...
  template <
      typename F, std::size_t Dim, typename T, typename... Tensors,
 ...
 *  @return an tensor of type T with elements described by \f$ f\left(\textbf{t}_{ij...}, \ldots\right) \f$
 *
 *  Applies a function elementwise between many tensors.
 */
  template <
      typename F, typename T, typename... Tensors,
      std::size_t Dim, std::size_t... Rest,
      typename U = std::invoke_result_t<F, T, typename Tensors::value_type...>>
  requires all_same<T, Tensors...>
  constexpr tensor<U, Dim, Rest...> elementwise(F f, const tensor<T, Dim, Rest...> &t, const Tensors &...tensors) {
  ...
  template <typename U, typename F, typename T, typename... Args>
  requires all_same<std::size_t, Args...>
  constexpr decltype(auto) generate(F &&f, Args... args) {
    tensor<U, 1> generated = {};
    return std::apply(f, std::forward_as_tuple(args...));
  }

  template <typename U, typename F, typename T, std::size_t Dim, std::size_t... Rest, typename... Args>
  requires all_same<std::size_t, Args...>
  constexpr decltype(auto) generate(F &&f, Args... args) {
 ...
 *  @return a tensor with type matching the return type of f such that \f$ \textbf{t}_{ij...} = f(i, j, ...) \f$
 *
 *  Generates a tensor as a function of its indices.
 */
  template <typename U, typename F, typename T, std::size_t Dim, std::size_t... Rest>
  requires std::is_invocable_r_v<U, F, Dim, Rest...>
